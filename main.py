...
class TaskManager:
    ...
    def get_task_tree(self) -> Dict:
        ...
        def build_tree(task_id):
            ...
            if task.children_ids:
                tree["children"] = [build_tree(child_id) for child_id in task.children_ids]
...
"""
    ...
    def run(self, initial_task: str):
        ...
        while not self.task_manager.all_tasks_completed():
            ...
            # 태스크 진행 중으로 상태 변경
            self.task_manager.update_task(next_task.task_id, status=TaskStatus.IN_PROGRESS)
            ...
            if task_type == "decomposition":
                ...
                # 현재 태스크 완료로 표시
                self.task_manager.update_task(next_task.task_id, status=TaskStatus.COMPLETED)
                print(f"태스크 분해: {next_task.description} -> {len(subtasks)}개의 서브태스크")
                ...
                self.task_manager.update_task(
                    next_task.task_id,
                    result=result,
                    status=TaskStatus.COMPLETED if "error" not in result else TaskStatus.FAILED
                )
